...
import { 
  faTachometerAlt, faUsers, faIdCard, faChalkboardTeacher, 
  faFileAlt, faCalendarDay, faCalendarAlt, faSyncAlt, faClock, faUserCog 
} from "@fortawesome/free-solid-svg-icons";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';

const Sidebar = () => {
  const [selected, setSelected] = useState(""); // Current selected item state
  const [reportDropdownOpen, setReportDropdownOpen] = useState(false); // State for the report dropdown
  ...
  return (
    <div className={`fixed inset-y-0 left-0 w-64 bg-black text-white overflow-y-hidden transition-transform duration-300 ${isVisible ? 'translate-x-0' : '-translate-x-full'}`}>
              ...
              <li>
                ...
                >
                  ...
                  <svg
                    ...
                    className={`h-4 w-4 ml-auto transition-transform duration-200 ${reportDropdownOpen ? "rotate-180" : "rotate-0"}`}
                    viewBox="0 0 20 20"
                    fill="none"
                    stroke="currentColor"
                  >
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 7l7 7 7-7" />
                  </svg>
                </div>
                {reportDropdownOpen && (
